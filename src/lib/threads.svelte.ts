...
interface Threads {
	...
	_addInitialData: (
		data: [typeof api.threads.get._returnType, typeof api.threads.pinned._returnType] | undefined
	) => void;
	prevPage: () => void;
	nextPage: () => void;
...
class ThreadsClass implements Threads {
	...
	private queryAll = $derived(
		useQuery(
			api.threads.get,
			{ paginationOpts: { cursor: this.cursor, numItems: PAGE_SIZE } },
	...
	searchQuery = $state('');
	private searchQueryRes = $derived(
		useQuery(api.threads.search, {
			searchQuery: this.searchQuery.length >= 2 ? this.searchQuery : null
		})
	);
	searchResults = $derived<Doc<'threads'>[] | null>(
		this.searchQuery.length >= 2 ? (this.searchQueryRes.data ?? []) : null
	);
